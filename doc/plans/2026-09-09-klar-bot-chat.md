...
The left navigation contains Bot-Chats, Posteingang, Armaturenbrett and Neue Aufgabe. Neue Aufgabe opens a single text field with a send arrow: no title, project, status, upload, automation, fullscreen or priority selectors. It always creates highest-priority (`critical`) work, generating the required task title from the first line. A bot explicitly named at the beginning (for example, `Aufgabe für Atlas, ...`, `Atlas: ...` or `@Atlas ...`) is resolved within the current company; otherwise the company's unique CEO receives the work for coordination. Unknown, ambiguous or unavailable recipients produce an error and retain the draft, never silently redirect to a different bot. Existing work is not interrupted and no control-plane policy is bypassed. Inbox and dashboard use the existing company-scoped routes. The mobile menu exposes the same destinations plus Bot hinzufügen and account settings. Bot hinzufügen is kept out of the left navigation.
